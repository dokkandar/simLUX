...
- [x] **PER/TAN unresponsive during edit ops** (bug 2, general): `find_all_snaps` was fed only `pending.last()` as the "from" anchor, which is empty during move/copy/STRETCH/mirror/rotate point picks → PER/TAN never computed. Now fed `card_anchor()` (returns the op's base/pivot, or draw pending). Fixes PER during stretch dest AND the same latent bug in move/copy/mirror/rotate.
